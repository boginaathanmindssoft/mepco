...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		if(!$ajax)
		{
			...
			$data->list_view = $this->_theme_view('custom_list.php',$data,true);
			$this->_theme_view('custom_list_template.php',$data);
		}
		else
		{
			$this->set_echo_and_die();
			$this->_theme_view('custom_list.php',$data);
		}
	...
	protected function showEditProfileForm($state_info)
	{
		$this->set_js_lib($this->default_javascript_path.'/'.grocery_CRUD::JQUERY);
		...
		$this->_theme_view('profile_edit.php',$data);
		$this->_inline_js("var js_date_format = '".$this->js_date_format."';");
		...
		if($field_info->extras == 'text_editor')
		{
			...
			switch ($editor) {
				case 'ckeditor':
					...
					$this->set_js_config($this->default_javascript_path.'/jquery_plugins/config/custom.jquery.ckeditor.config.js');
				break;
	...
	protected function get_datetime_input($field_info,$value)
	{
		...
		if(!empty($value) && $value != '0000-00-00 00:00:00' && $value != '1970-01-01 00:00:00'){
			...
			$datetime = $date.substr($value,10, 6);
		}
		else
		{
			$datetime = '';
	...
	protected function get_dropdown_input($field_info,$value)
	{
		...
		if(isset($field_info->extras['default']) &&
			!empty($field_info->extras['default']) &&
			$field_info->extras['default'] != null)
		{
			$value = $field_info->extras['default'];
			unset($field_info->extras['default']);
		}
		$onchange = '';
		if($field_info->name == "gallery_type"){
			$onchange = " onchange='return onchangeGalleryType(this);' ";
		}
		$this->load_js_chosen();
		...
		$input = "<select".$onchange." id='field-{$field_info->name}' name='{$field_info->name}' class='chosen-select' data-placeholder='".$select_title."'>";
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		if($field_info->name == "image_path"){
			$accept = 'accept="image/*"';
		}
		elseif($field_info->name == "file_url"){
			$accept = 'accept="text/plain, application/pdf"';
		}
		else{
			$accept = '';
		}
		...
		$input = '<span class="fileinput-button qq-upload-button" id="upload-button-'.$unique.'" style="'.$uploader_display_none.'">
			...
			<input type="file" name="'.$this->_unique_field_name($field_info->name).'" class="gc-file-upload" rel="'.$this->getUploadUrl($field_info->name).'" id="'.$unique.'" '.$accept.'>
...
class Grocery_CRUD extends grocery_CRUD_States
{
	 ...
	 * @param array|string $extras
	 */
	public function field_type($field , $type, $extras = null, $default = null)
	{
	...
	public function render()
	{
		...
		switch ($this->state_code) {
			...
			case 3://edit
				...
				$state_info = $this->getStateInfo();
				if($this->custom_state_code == 21){
					$this->showEditProfileForm($state_info);
				}
				else{
