...
class All_Applications extends CI_Controller {
	...
	public function page()
	{
		...
		$data = array();
		$data["content"] = "application_category_list";
		$data["active_menu"] = "applications";

		$data["meta_title"] = "All Applications | Mepco - Metal Powder Company";
       ...
       $total_records = $this->products_model->get_total_products(1);
        if ($total_records > 0)
        {
	        // get current page records
			$data["results"] = $this->products_model->get_current_page_records($limit_per_page, $page*$limit_per_page, 1);
			$config = array();

			$config["base_url"] = base_url('all-applications/page');
